...
import {addPhrase, removePhrase, reorderPhrase, updatePhrasePoints, updatePhraseText,} from "../_lib/gameActions";
import {parseSpotifyTrackId} from "../_lib/spotify";
import {type GameState, MAX_WORDS, MIN_WORDS} from "../_lib/types";
import {deleteCustomSong, getAllSongs, saveCustomSong, type SavedSong} from "../_lib/preloadedSongs";
...
export default function PhraseQueueEditor({
    ...
    const handleSaveEdit = async (id: string) => {
        ...
        if (wordCount < MIN_WORDS || wordCount > MAX_WORDS) {
            setEditError(`Frasen må ha mellom ${MIN_WORDS} og ${MAX_WORDS} ord (nå: ${wordCount}).`);
            return;
        }
        setEditError(null);
    ...
    const submitManual = async () => {
        ...
        const title = songTitle.trim() || trimmedDraft.split(" ").slice(0, 3).join(" ") + "...";
        saveCustomSong({
            title,
            phrase: trimmedDraft,
    ...
    const handleAddFromArchive = async (song: SavedSong) => {
        await applyAction((s) => addPhrase(s, song.phrase, undefined, song.spotifyTrackId));
    };
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {activeTab === "manual" ? (
                <div className="flex flex-col gap-3 bg-white/50 p-4 rounded-xl border border-gray-200">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        />
                        {spotifyError && <p className="text-red-500 text-sm mt-0.5">{spotifyError}</p>}
                    </div>
                      ...
                      <span className="font-semibold text-xs text-gray-900 truncate">
                        {song.title}
                          ...
                          <Music size={12} className="text-green-600 shrink-0"/>
                                            ...
                                            {song.isCustom && (
                                                <span
                                                    className="text-[9px] bg-amber-100 text-amber-800 font-bold px-1.5 py-0.2 rounded-full">
                          Lagret
                        </span>
                                            )}
                                        </div>
            ...
            <div className="flex items-center justify-between border-t border-gray-200/60 pt-3 mt-1">
                ...
                <p className="text-[10px] text-gray-400">Flytt med pilene eller rediger strofer</p>
            </div>
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-0.5 mr-0.5">
                                <button
                                    ...
                                    onClick={() => applyAction((s) => reorderPhrase(s, p.id, "up"))}
                                ...
                                <button
                                    ...
                                    onClick={() => applyAction((s) => reorderPhrase(s, p.id, "down"))}
                                    className="text-gray-400 hover:text-gray-700 disabled:opacity-20 p-0.5"
                                    title="Flytt ned"
                                >
                                    <ChevronDown size={12}/>
                ...
                {i + 1}
                            ...
                            {editingId === p.id ? (
                                <div className="flex-1 flex flex-col gap-1 min-w-0">
                                    ...
                                    {editError &&
                                        <p className="text-[10px] text-red-500 font-semibold mt-0.5">{editError}</p>}
                                </div>
                            ) : (
